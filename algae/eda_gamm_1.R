...
pairs<-pairs(totals[,c(4:11,13)])
ggsave(plot=pairs,filename='data/pairs.png',height = 18, width =16, units = 'cm')
...
all_smooth<- ggplot(totals.long, aes(sampledate, value, color=variable))+
  geom_point()+
  geom_smooth(aes(group=variable))+
  #geom_line(aes(group=variable))+
  facet_wrap(~variable, scales='free')
ggsave(plot=all_smooth,filename='data/all_smooth.png',height = 18, width =16, units = 'cm')


all_smooth_year<- ggplot(totals.long, aes(year, value, color=variable))+
  geom_point()+
  geom_smooth(aes(group=variable))+
  #geom_line(aes(group=variable))+
  facet_wrap(~variable, scales='free')
ggsave(plot=all_smooth_year,filename='data/all_smooth_year.png',height = 18, width =16, units = 'cm')
...
winter_smooth_years<- ggplot(winter, aes(year, value, color=variable))+
  geom_point()+
  geom_smooth(aes(group=variable))+
  #geom_line(aes(group=variable))+
  facet_wrap(~variable, scales='free')
ggsave(plot=winter_smooth_years,filename='data/winter_smooth_year.png',height = 18, width =16, units = 'cm')

winter_lm_smooth_years<- ggplot(winter, aes(year, value, color=variable))+
  geom_point()+
