...
#join3 <- fuzzy_left_join(join, totalbv2, by = c("intchemsDate" = "datePlus1", "intchemsDate" = "dateMinus1"), 
#                       match_fun = list(`<=`, `>=`)) 
...
surfchlor2=data.frame(surfchlorDate= c(as.Date(surfchlor$sampledate),surfchlor= c(surfchlor$chlor)))%>% 
  mutate(datePlus1 = surfchlorDate + 1) %>% mutate(dateMinus1 = surfchlorDate - 1)

totalbv2 = data.frame(totalbvDate= c(as.Date(totalbv$sampledate)), genus=c(totalbv$Genus), CellBioVol=c(totalbv$CellBioVol))


surftotalbv<- fuzzy_left_join(surfchlor2, totalbv2, by = c("totalbvDate" = "datePlus1", "totalbvDate" = "dateMinus1"), 
                         match_fun = list(`<=`, `>=`)) 
...
totalbv2 = data.frame(totalbvDate= c(totalbv$sampledate), totalbv$Genus, totalbv$CellBioVol)


join= data.frame(joinDate= c(join$sampledate, join$intchems.wtemp)%>% 
                   mutate(datePlus1 = joinDate + 1) %>% mutate(dateMinus1 = joinDate - 1))


join3 <- fuzzy_left_join(join, totalbv2, by = c("totalbvDate" = "datePlus1", "totalbvDate" = "dateMinus1"), 
                         match_fun = list(`<=`, `>=`)) 
